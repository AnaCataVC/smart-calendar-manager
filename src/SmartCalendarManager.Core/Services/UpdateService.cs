...
public interface IUpdateService : IDisposable
{
    ...
    Task<string> DownloadUpdateAsync(string downloadUrl, string? fileName = null, IProgress<double>? progress = null, CancellationToken cancellationToken = default);
...
public class UpdateService : IUpdateService
{
    ...
    private const string GitHubRepoName = "smart-calendar-manager";
    ...
    private void ConfigureHttpClient()
    {
        if (!_httpClient.DefaultRequestHeaders.Contains("User-Agent"))
        {
            _httpClient.DefaultRequestHeaders.UserAgent.Add(
                new ProductInfoHeaderValue("SmartCalendarManager", _currentVersion));
        }
    ...
    public async Task<UpdateInfo> CheckForUpdatesAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            _logger?.LogInformation("Checking for updates from {Url}...", LatestReleaseApiUrl);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var tempDir = Path.Combine(Path.GetTempPath(), "SmartCalendarManager_Updates");
        Directory.CreateDirectory(tempDir);

        var safeFileName = string.IsNullOrWhiteSpace(fileName) ? "SmartCalendarManager-Setup.exe" : fileName;
